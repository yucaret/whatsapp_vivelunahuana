...
def generar_respuesta_chatgpt(user_message, number, espedido=False):
    ...
    if espedido:
      ...
      messages.append(
                        ...
                        4) categoria; \
                        5) servicio, este elemento se refiere al servicio que pidio información; \
                        6) precio total.'},
                    )
    try:
...
def text_Message_al_gerente(number, diccionarioConforme):
    text = 'Hola Victor, el cliente ' + str(diccionarioConforme['nombre completo del cliente']) + ' ' \
           'con celular ' + str(diccionarioConforme['celular']) + ' y correo ' + str(diccionarioConforme['correo electronico']) + ' ' \
           'a solicitado un ' + str(diccionarioConforme['categoria']) + ' con el detalle de ' + str(diccionarioConforme['servicio']) + ' ' \
           'y precio total: ' + str(diccionarioConforme['precio total']) + '.'
...
def get_chat_from_csv(number):
    messages = []
    print('inicio de get_chat_from_csv')
    ...
    try:
        filename_conversaciones = 'conversaciones' + '_' + str(number) + '.csv'
        with open(filename_conversaciones) as file:
            reader = csv.DictReader(file)
            print('conversaciones.csv: ' + filename_conversaciones)
    ######
            for row in reader:
    ...
    ######
        ...
        print("sin archivo, sin historial: " + str(e))
    ######
